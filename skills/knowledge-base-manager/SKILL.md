...
本 Skill 不依赖外部告知 KB 位置，自带四层定位逻辑：
...
KB_PATH=
  ...
  3. $CWD/knowledge-base/（次高）
  4. $(find . -maxdepth 3 -name ".index.jsonl" -path "*/knowledge-base/*" 2>/dev/null | grep -v _compiled | head -1 | xargs dirname)（兜底）
```

调用时优先使用环境变量，未设置时自动从 CWD 向上查找，确保任何项目下开箱即用。
...
## 扩展：LangExtract 剧本结构化集成（C24，2026-04-30）

### 新增能力

LangExtract 用于将小说文本深度结构化，输出多 Agent 可用的结构化数据。

### 集成场景

| 场景 | 触发条件 | 输出 |
|------|---------|------|
| P1 脚本生成前 | 收到原著文本 | character/dialogue/scene/sfx JSONL |
| 长篇一致性验证 | S4 验收前 | grounding verification report |
| 知识库补充 | 提取完成后 | 新增 character/scene 条目 |

### 数据格式

LangExtract 提取结果（JSONL）：
```json
{
  "extraction_class": "character|dialogue|scene|sfx",
...
```bash
# 预提取脚本（需安装 langextract）
bash skills/knowledge-base-manager/scripts/langextract-pre.sh "<原著作名>" "<章节>" "<文本路径或URL>"

# 溯源验证脚本
bash skills/knowledge-base-manager/scripts/grounding-verify.sh "<提取结果.jsonl>"
```

### 约束

- char_interval 为 None 的提取物必须过滤（来自 LLM 内部知识，非原文）
- 每个 extraction 必须有 char_interval 才可入库
- 溯源验证失败 → 不允许进入下一节点

### 相关 Skill

- [[audio-comic-workflow]]：P1 脚本生成前置
- [[comic-style-consistency]]：character visual features
- [[supervision-anti-drift]]：grounding verification
- [[self-optimizing-yield]]：extraction quality tracking


---

## 扩展：pi-mono 多模型调度架构参考（2026-04-30）

### 相关项目

**badlogic/pi-mono**：AI agent toolkit，多模型统一调度 + Agent 状态管理

| 包 | 功能 | 借鉴点 |
|---|------|--------|
| @mariozechner/pi-ai | 多供应商 LLM API 统一接口 | 多模型动态切换（Claude/MiniMax/oLLM） |
| @mariozechner/pi-agent-core | Agent 运行时 + 状态管理 | 多 Agent 协作时的上下文维护 |
| @mariozechner/pi-tui | Terminal UI 库 | 流水线监控仪表盘 |

### 集成场景

| 场景 | 参考 pi-mono 实现 |
|------|-----------------|
| 多模型切换 | pi-ai 的统一 API 接口，支持 OpenAI/Anthropic/Google |
| 长流程状态 | pi-agent-core 的状态管理，避免上下文丢失 |
| 监控面板 | pi-tui 的 differential rendering，可构建实时仪表盘 |
...
- 关联约束：hc-ap1（本地明文永远保留）/ hc-ap3（禁止自动删除）/ c-dev（项目目录约束）
